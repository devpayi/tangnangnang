...
const groupColor = { A: '#dc2626', B: '#d97706', C: '#7c6fd6', NEW: '#64748b' }
// การ์ดโทนม่วงชมพูอ่อนมากๆ — หน้านี้ข้อมูลเยอะ เลยเบาสีลงเป็นพิเศษกันลายตา (ขอบ/พื้นหลังจางกว่าหน้าอื่น)
const panel = {
  background: 'rgba(255,255,255,.94)',
  backdropFilter: 'blur(10px) saturate(140%)', WebkitBackdropFilter: 'blur(10px) saturate(140%)',
  border: '1px solid #ece4f8', borderRadius: 20,
  boxShadow: '0 8px 22px rgba(142,117,255,.08)',
}
const input = { width: 76, border: '1px solid #e3d9f5', borderRadius: 8, padding: '8px 9px', fontSize: 14, textAlign: 'right', outline: 'none', boxSizing: 'border-box' }
...
export default function PlannerControl({ onNavigate }) {
  ...
  return <div className="planner-control-page" style={{ display: 'grid', gap: 14 }}>
    <div style={{ ...panel, padding: 18, background: 'linear-gradient(135deg,#f5f0fd,#ffffff 70%)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, flexWrap: 'wrap' }}>
      <div>
    ...
    <div style={{ ...panel, padding: '12px 15px', display: 'flex', alignItems: 'center', gap: 18, flexWrap: 'wrap' }}>
      ...
      <QuickStat label="Manpower" value="4 คน" color="#7c6fd6" />
    ...
    <section style={{ ...panel, overflow: 'hidden' }}>
      ...
      <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 900, tableLayout: 'fixed', borderCollapse: 'collapse', fontSize: 14 }}>
        ...
        <thead><tr style={{ background: '#f7f2fc', color: '#5c5578' }}>{['สินค้า','FG','เฉลี่ย/วัน','เผื่อวัน','กันพุ่ง','แนะนำฟีด','ฟีดวันนี้','ยังขาด',''].map((head, index) => <th key={`${head}-${index}`} style={{ padding: '11px 8px', whiteSpace: 'nowrap', textAlign: index === 0 ? 'left' : 'center' }}>{head}</th>)}</tr></thead>
        <tbody>{visible.map((item) => <tr key={item.id} style={{ borderTop: '1px solid #efe8fa', background: item.remaining > 0 ? '#fff' : 'rgba(240,253,247,.5)' }}>
          ...
          <td style={{ ...td, textAlign: 'center', color: '#6E56CF', fontWeight: 900 }}>{fmt(item.need)}</td>
...
function ProductModal({ modal, onClose, onSave, onEdit, onCreateOt }) {
  ...
  return <div onMouseDown={onClose} style={{ position: 'fixed', inset: 0, zIndex: 2500, background: 'rgba(15,23,42,.34)', backdropFilter: 'blur(3px)', display: 'grid', placeItems: 'center', padding: 18 }}>
    <div onMouseDown={(event) => event.stopPropagation()} style={{ width: isDetail ? 500 : 590, maxWidth: '100%', maxHeight: '90vh', overflowY: 'auto', background: '#fff', borderRadius: 18, boxShadow: '0 28px 80px rgba(15,23,42,.25)' }}>
      <div style={{ padding: '17px 19px', borderBottom: '1px solid #efe8fa', display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'start' }}>
...
const primaryButton = { border: 0, borderRadius: 10, padding: '9px 11px', background: 'linear-gradient(135deg, #8E75FF, #6E56CF)', boxShadow: '0 8px 18px rgba(110,86,207,.28)', color: '#fff', fontWeight: 850, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }
const secondaryButton = { border: '1px solid #e3d9f5', borderRadius: 9, padding: '9px 11px', background: '#fff', color: '#6E56CF', fontWeight: 850, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7 }
const formInput = { width: '100%', boxSizing: 'border-box', border: '1px solid #e3d9f5', borderRadius: 9, padding: '9px 10px', background: '#fff', color: '#0f172a', fontSize: 14, outline: 'none' }
const detailLabel = { color: '#64748b', fontSize: 11, fontWeight: 800 }
const simpleTab = (active) => ({ border: `1px solid ${active ? '#5ca8df' : '#d7e3ef'}`, background: active ? '#eaf5ff' : '#fff', color: active ? '#6E56CF' : '#64748b', borderRadius: 9, padding: '8px 11px', fontWeight: 850, cursor: 'pointer' })
