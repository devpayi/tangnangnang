...
export default function FeedProducts() {
  ...
  useEffect(() => {
    ...
    fetch('/api/feed-summary')
      ...
      .then((data) => {
        if (!active) return
        if (!data?.success) throw new Error(data?.error || 'โหลดช่วงข้อมูลไม่สำเร็จ')
        setDashData(data)
        setError('')
      })
  ...
  const onRetry = () => setRangeReload((value) => value + 1)
  ...
  useEffect(() => {
    ...
    fetch(`/api/feed-summary?startDate=${startDate}&endDate=${endDate}`)
  ...
  if (loading || configLoading || (!effectiveMonth && !error)) {
    return <div style={{ padding: 36, textAlign: 'center', color: '#8b7fa8' }}>กำลังโหลดรายการสินค้าที่ต้องฟีด...</div>
  }
  ...
  if (error && !dashData) {
    return (
      <div style={{ padding: 36, textAlign: 'center', color: '#b91c1c', background: '#fff', border: '1px solid #fecaca', borderRadius: 16 }}>
        ...
        <button type="button" onClick={onRetry} style={{ marginLeft: 10, border: 0, background: 'transparent', color: '#7c6fd6', fontWeight: 800, cursor: 'pointer' }}>ลองใหม่</button>
      </div>
    )
  }
  ...
  return (
    <div style={{ display: 'grid', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'end', gap: 12, flexWrap: 'wrap' }}>
        <label style={{ display: 'grid', gap: 5, minWidth: 220, color: '#5c5578', fontSize: 12, fontWeight: 800 }}>
          เดือนที่ใช้อ้างอิง
          <select
            value={effectiveMonth}
            onChange={(event) => setSelectedMonth(event.target.value)}
            style={{ minHeight: 42, border: '1px solid #e3d9f5', borderRadius: 9, padding: '8px 11px', background: '#fff', color: '#2a1f42', fontWeight: 800 }}
          >
      ...
      <section style={{ background: '#fff', border: '1px solid #ece4f8', borderRadius: 16, overflow: 'hidden', boxShadow: '0 10px 28px rgba(142,117,255,.08)' }}>
        <div style={{ padding: '15px 18px', borderBottom: '1px solid #ece4f8', display: 'flex', alignItems: 'center', gap: 9 }}>
          <PackageCheck size={19} color="#7c6fd6" />
          <div>
            <b style={{ color: '#2a1f42' }}>สินค้าที่ต้องฟีด</b>
            <div style={{ color: '#8b7fa8', fontSize: 12, marginTop: 2 }}>จำนวนชิ้นที่ออกในเดือนที่เลือก รวมยกเลิก/ตีคืน · แสดงเฉพาะสินค้าที่ใช้งานใน Planner Control</div>
          </div>
        </div>

        {monthLoading ? (
          <div style={{ padding: 36, textAlign: 'center', color: '#8b7fa8' }}>กำลังโหลดข้อมูลเดือนที่เลือก...</div>
        ) : monthError ? (
          <div style={{ padding: 36, textAlign: 'center', color: '#b91c1c' }}>
            {monthError}
            <button type="button" onClick={() => setMonthReload((value) => value + 1)} style={{ marginLeft: 10, border: 0, background: 'transparent', color: '#7c6fd6', fontWeight: 800, cursor: 'pointer' }}>ลองใหม่</button>
          </div>
        ) : rows.length ? (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', maxWidth: 720, tableLayout: 'fixed', borderCollapse: 'collapse' }}>
              <colgroup>
                <col style={{ width: 64 }} />
                <col style={{ width: 130 }} />
                <col />
              ...
              <thead>
                <tr style={{ background: '#f7f2fc', color: '#6b5f8a', fontSize: 12 }}>
              ...
              <tbody>
                {rows.map((row, index) => (
                  <tr key={row.master_sku} style={{ borderTop: '1px solid #f5f0fd' }}>
                    <td style={cell}>{index + 1}</td>
                    <td style={{ ...cell, textAlign: 'left', fontFamily: 'monospace', fontWeight: 800, color: '#7c6fd6' }}>{row.master_sku}</td>
                    <td style={{ ...cell, textAlign: 'left', color: '#4a4460', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row.display_name || row.master_sku}</td>
                    <td style={{ ...cell, textAlign: 'right', fontWeight: 900, color: '#2a1f42' }}>{fmt(row.qty)} ชิ้น</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={{ padding: 36, textAlign: 'center', color: '#8b7fa8' }}>ไม่พบข้อมูลสินค้าที่ต้องฟีดในช่วงวันที่เลือก</div>
        )}
      </section>
    </div>
  )
...
function Stat({ label, value }) {
  return (
    <div style={{ background: '#fff', border: '1px solid #ece4f8', borderRadius: 14, padding: '14px 16px' }}>
      <div style={{ color: '#8b7fa8', fontSize: 12 }}>{label}</div>
      <div style={{ color: '#2a1f42', fontSize: 22, fontWeight: 900, marginTop: 5 }}>{value}</div>
    </div>
  )
}

const head = { padding: '11px 14px', textAlign: 'center', fontWeight: 800, whiteSpace: 'nowrap' }
const cell = { padding: '10px 14px', textAlign: 'center', fontSize: 13 }
